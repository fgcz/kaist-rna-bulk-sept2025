...
kallistoFilePaths <- Sys.glob(file.path(countsDirectoryToUse, "*txt"))
```
...
Let's set some thresholds that we will use going forward to filter the DE result. Namely, we will set the p-value, log2-fold, and adjusted p-value thresholds above/below which we will consider genes to be differentially expressed.
...
Let's next visualise the data in a volcano plot. This type of plot summarises the p-values against log-fold changes as a scatterplot, resulting in a figure resembling a volcano. We will use the packaged `EnhancedVolcano` [@EnhancedVolcano] since it works well out of the box.
...
1.  Put the gene names in question to the "selectLab" argument and re-run that line.
    ...
    ESRP1 is upregulated (positive logFC) while ZEB1 is downregulated (negative logFC).
:::
...
In this section, we would like to take our results from the differential expression and identify genes in the result which are functionally related. To this end, this so-called enrichment analysis can help us. As mentioned in the lecture, there are several tools in R that allow us to perform this analysis, but we will make use of clusterProfiler [@clusterProfiler]. Its documentation can be found [here](https://yulab-smu.top/biomedical-knowledge-mining-book/enrichment-overview.html). Its main strengths lie in a number of built-in visualisations we will find helpful in interpreting the data.
