...
fcFiles <- Sys.glob("~/work/results/3_quant/featureCounts/*[0-9].txt")
if(length(fcFiles) > 0) {
...
1.  FeatureCounts typically runs quite quickly on subsampled data like this, but keep in mind that the difficult and time-intensive step of alignment has already been done. Kallisto takes on the order of a minute or so for these data. Nonetheless, both tools perform quickly on datasets of this size. Keep in mind that featureCounts requires .bam files which we generated in a previous step with STAR, while Kallisto works on the fastqs themselves. Alignment with STAR can take a long time, especially with large genomes.
...
3.  FeatureCounts outputs integer counts, which is appropriate for most differential expression analysis tools that expect discrete count data. This contrasts with some other quantification methods that might output continuous values that need to be rounded, like the abundance estimates from Kallisto.
:::

## CountQC

Now that we know how to generate counts based on some downsampled data, let's move on to how we can QC counts. For this, we switch again to the **full data**, which has been processed for us in an analogous way to the steps above.
