...
```
    ...
    2. The rows are the same, but they are in a different order.

    3. The kallisto counts (est_count) are floating point numbers. This may become a problem when it comes time to load these counts for differential expression, since most of the available tools expect the counts to be discreet. In such cases we may have to round the counts.
...
meta <- data.frame(
  Condition=as.factor(c(rep("Glucose", 4), rep("GlycEth", 4))),
  row.names=c(paste0("G", 1:4), paste0("GE", 1:4))
)
```

First, we load the counts and combine them into a single data-frame.
...
Let's look at the count statistics first.
...
```{r}
mds <- limma::plotMDS(normCounts, plot=FALSE)
...
```{r}
d <- as.dist(1-cor(normCounts, use="complete.obs"))
hc <- hclust(d, method="ward.D2")
plotDendroAndColors(hc, sampleColours, autoColorHeight=TRUE, hang = -0.1)
```
