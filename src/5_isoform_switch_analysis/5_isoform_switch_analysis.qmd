...
```{r dexseq}
# Subsampling so we only get results with very high differential isoform fraction
aSwitchList2 <- subsetSwitchAnalyzeRlist(
    switchAnalyzeRlist = aSwitchList,
    subset = abs(aSwitchList$isoformFeatures$dIF) > 0.4
)
...
aSwitchList2 <- isoformSwitchTestDEXSeq(
