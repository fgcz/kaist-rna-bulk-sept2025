...
There are different approaches supported in `IsoformSwitchAnalyzeR`. We are using `DEXSeq`. This is a step that would take long and exhaust the computational resources of this session. Therefore we will subsample the data and proceed with only a subset of our data:
...
tibble::as_tibble(head(aSwitchList_sub$AlternativeSplicingAnalysis, 10))
...
3.  Only genes with isoform switches would be included (filtering by the parameters `alpha` and `dIFcutoff`). The default value is `TRUE` but the documentation incorrectly claims it's `FALSE`!
